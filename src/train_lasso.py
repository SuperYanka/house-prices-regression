...
model = Pipeline(steps=[
    ...
    ('regressor', Lasso(alpha=0.0005, max_iter=10000))
])

params = {
    'regressor__alpha': [0.1, 0.01, 0.001, 0.0005, 0.0001]
}
...
# Сохраняем модель
model_path = os.path.join(BASE_DIR, '..', 'models', 'lasso.pkl')
joblib.dump(model, model_path)

# Сохраняем метрики
metrics_path = os.path.join(BASE_DIR, '..', 'models', 'lasso_metrics.json')
